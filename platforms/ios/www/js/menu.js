...
function loadRestaurantMenu() {
   ...
   for (var i = 0; i < categoryList.length; i++) {
      ...
      addCollapsableRow("category_" +category.id, category.name, "accordion", category.items.length);
...
function addComboItems(itemGroup, item) {
   ...
   for (var i = 0; i < itemGroup.length; i++) {
      ...
      for (var k = 0; k < group.items.length; k++) {
         ...
         var itemLabelStart = "<label class=\"my-panel checkbox-btn btn nohover btn-default\" " +
         ...
         var itemLabelEnd = "</label>";
         if (currentItem.condiments.length > 0) {
            itemLabelStart += "<div id=\"group_" + group.id + "_item_" + currentItem.id + "_condiments\" class=\"groupItemsCondiments" + viewable + " checkbox-grid\">";
            itemLabelStart += getCondimentHtml(currentItem.condiments, currentItem, true, "group_" + group.id + "_item_" + currentItem.id + "_");
            itemLabelEnd = "</div></label>";
         }
...
function addCondimentsToMenuItem(condiments, item) {
   ...
   var htmlEnd = "<br>" +
      "<div class=\"checkbox-grid\" style=\"text-align: center;\"><p><button type=\"button\" onclick=\"incrementCart('" + item.id + "'); return false;\" class=\"btn btn-primary\"><i class=\"fa fa-plus\"></i></button>&nbsp;&nbsp;&nbsp;" +
      "<span class=\"quantity\" id=\"itemid_" + item.id + "_quantity\">1</span>&nbsp;&nbsp;&nbsp;" +
      "<button type=\"button\" onclick=\"decrementCart('" + item.id + "'); return false;\" class=\"btn btn-danger\"><i class=\"fa fa-minus\"></i></button></p>" +
      "<p><button type=\"button\" class=\"btn btn-success\" id=\"btnAdd\" onclick=\"addToCart('" + item.id + "'); return false;\" >" +
...
function addItemToCategory(id, name, appendTo, price, description) {
   var div = "<div class=\"panel panel-default\">" +
      ...
      "     <div class=\"my-panel panel-body\">"+ description + "<div  id=\"" + id + "_condiments\"></div>" +
      " </div></div>";
      // alert(div);
   $("#" + appendTo).append(div).trigger('create');
}

function addCollapsableRow(id, name, appendTo, itemCount) {
   var div = "<div class=\"panel panel-default\">" +
      ...
      "     <div class=\"my-panel panel-body\" ><div class=\"panel-group\" id=\"" + id + "_accordion\"></div>" +
      " </div></div>";
      // alert(div);
   $("#" + appendTo).append(div).trigger('create');
}
